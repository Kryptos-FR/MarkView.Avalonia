...
- Coverage via `Microsoft.Testing.Extensions.CodeCoverage` (`testconfig.json`, passed explicitly with `--config-file` in the workflow — routing it through `TestingPlatformCommandLineArguments` in `Directory.Build.props` is NOT honored by this SDK's `dotnet test` MTP mode), reported by `.github/workflows/coverage.yml`.
